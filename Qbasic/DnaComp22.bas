...
DECLARE SUB Compressor (DaFile$, InFile$)
...
'$STATIC
DIM SHARED ParoleC$(214)
DIM SHARED Cl(7)
DIM SHARED ab$(400)
DIM SHARED Frase$(2000)
'$DYNAMIC
...
COLOR Cl(1), Cl(2)
Finestra LargP, AltP
Center AltezzaP, " DNA Compressor 2.0 ", 2
COLOR 4, 7
LOCATE 13, 16
PRINT STRING$(50, " ")
Esecuzione
...
Errori:
    ...
    COLOR Cl(1), Cl(7)
    Finestra 26, 4
    Center AltezzaP + 2, "Errore numero:" + STR$(ERR), 4
    SELECT CASE ERR
    ...
    END SELECT
    Center AltezzaP + 3, TipoErr$, 4
    Center AltezzaP + 4, "Riavviare DNA 2.0", 4
END
RESUME
...
WHILE EOF(1) = 0
    ...
    FOR x = 1 TO LEN(a$)
        a = a + 1
        b$ = LTRIM$(STR$(ASC(RIGHT$(LEFT$(a$, a), 1))))
        IF LEN(b$) = 2 THEN b$ = "0" + b$
        IF LEN(b$) = 1 THEN b$ = "00" + b$
        Frase$ = Frase$ + b$
        IF LEN(Frase$) > 246 THEN PRINT #2, Frase$: Frase$ = ""
        GOSUB Linea
    NEXT
    Frase$ = Frase$ + "013"
        PRINT #2, Frase$
...
SUB Esecuzione
    STATIC DaEx$, Ex$
    F1$ = "Nome del file da comprimere: "
    F2$ = "Nome del file compresso: "
    ...
    Center AltezzaP + 4, " Compressione terminata", 2
    Center AltezzaP + 5, GDaEx$ + " ->" + GEx$, 2
    Center AltezzaP + 6, STR$(CINT(((VAL(GDaEx$) - VAL(GEx$)) / VAL(GEx$)) * 100)) + "%", 2
    SLEEP 8
    RUN
END SUB
