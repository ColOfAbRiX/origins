...
CONST AltezzaP = ((Asch / 2) - (AltP / 2))
       ...
       'Scritte Sfondo Bordo Errore Barre BordoExt
Col:
    DATA   15,     1,     7,    12,    14,     0
BN:
    DATA    7,     0,     7,    15,     7,     0
...
Colori
Finestra LargP, AltP
Center AltezzaP, " DNA Expander 1.0 ", 2
Esecuzione
END
...
SUB Colori
    ...
    IF SceltaCol$ = "BN" THEN
        RESTORE BN
        READ Cl(1), Cl(2), Cl(3), Cl(4), Cl(5), Cl(6)
    END IF
...
SUB Expander (DaFile$, InFile$)
    OPEN DaFile$ FOR INPUT AS #1
        LINE INPUT #1, Linea$
    a = 0
    FOR x = 1 TO LEN(Linea$) / 3
        a = a + 3
        b = b + 1
        a(b) = VAL(RIGHT$(LEFT$(Linea$, a), 3))
    NEXT
    b = 0
    NR = 1
    FOR x = 1 TO UBOUND(a)
        b = b + 1
        IF a(b) = 0 THEN EXIT FOR
        IF a(b) < Ascii THEN
            IF a(b) = 13 THEN NR = NR + 1
            Letter$ = CHR$(a(b))
            Frase$(NR) = Frase$(NR) + Letter$
        END IF
        c = 0
        IF a(b) >= Ascii THEN
            FOR x = 1 TO UBOUND(Dna$, 2)
                c = c + 1
                IF a(b) - Ascii = VAL(Dna$(1, c)) THEN Frase$(NR) = Frase$(NR) + Dna$(2, c) + CHR$(32)
            NEXT
        END IF
    NEXT
    CLOSE #1
